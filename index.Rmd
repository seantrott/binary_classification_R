...
As noted above, the core principle underlying SVMs is the idea of a **separating hyperplane**. In $p$-dimensional space, a hyperplane is a flat subspace of dimension $p - 1$. For example, in 2D space, a hyperplane would simply be a line, and the equation is given thus:
...
Personally, I had trouble understanding this latter constraint. But recall that our values for $Y$ are two classes coded as ${-1, 1}$. This means that any given observation $y_i$ could be either $-1$ or $1$--i.e., either positive or negative. Similarly, our predicted value $\hat{y_i}$ could  be either positive or negative. **Critically, this constraint requires that the sign of our predicted value matches the sign of our actual observation.** If the signs match, then the product $y_i * \hat{y_i}$ will always be positive, and thus has a greater chance of being larger than $M$. Whereas if the signs *don't* match, then the product $y_i * \hat{y_i}$ will be negative, meaning it's been classified on the wrong side of the margin.
